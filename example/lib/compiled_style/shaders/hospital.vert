...
const highp vec4 color = vec4(0.2025466666666667, 0.18260000000000004, 0.25739999999999996, 1.0);
const highp vec2 translate = vec2(0, 0);

void main() {
  highp float opacity = data_interpolate(opacity_start_value, opacity_end_value, hospital_ubo.opacity_stops.x, hospital_ubo.opacity_stops.y);
  v_opacity = opacity;
