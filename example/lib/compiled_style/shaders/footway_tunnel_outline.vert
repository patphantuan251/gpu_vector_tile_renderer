...
uniform FootwayTunnelOutlineUbo {
  vec2 width_stops;
} footway_tunnel_outline_ubo;
...
const highp vec4 color = vec4(0.0, 0.0, 0.0, 1.0);
...
void main() {
  float width = data_interpolate(width_start_value, width_end_value, footway_tunnel_outline_ubo.width_stops.x, footway_tunnel_outline_ubo.width_stops.y);
  v_width = width;
