...
void main() {
  highp vec4 color = data_interpolate(color_start_value, color_end_value, major_road_ubo.color_stops.x, major_road_ubo.color_stops.y);
  v_color = color;
  float width = data_interpolate(width_start_value, width_end_value, major_road_ubo.width_stops.x, major_road_ubo.width_stops.y);
  v_width = width;
