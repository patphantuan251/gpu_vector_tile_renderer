...
uniform MajorRailHatchingUbo {
  vec2 color_stops;
  vec2 width_stops;
  vec2 dasharray_size;
} major_rail_hatching_ubo;
...
void main() {
  highp vec4 color = data_interpolate(color_start_value, color_end_value, major_rail_hatching_ubo.color_stops.x, major_rail_hatching_ubo.color_stops.y);
  v_color = color;
  v_opacity = opacity;
  float width = data_interpolate(width_start_value, width_end_value, major_rail_hatching_ubo.width_stops.x, major_rail_hatching_ubo.width_stops.y);
  v_width = width;
  vec2 dasharray_size = major_rail_hatching_ubo.dasharray_size;
