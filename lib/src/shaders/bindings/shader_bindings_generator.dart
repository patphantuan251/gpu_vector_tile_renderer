...
String _generateShaderUboSetter(ParsedShader vertexShader, ParsedShader fragmentShader) {
  final o = StringBuffer();
  final ubos = {...vertexShader.ubos, ...fragmentShader.ubos};
...
String generateShaderBindings(List<ParsedShader> shaders) {
  ...
  for (final shader in vertexShaders) {
    // -- Class declaration and constructor --
    o.writeln('/// Generated bindings for the vertex shader `${shader.name}`');
    o.writeln('class ${shader.dartClassName} extends VertexShaderBindings {');
    o.writeln('  ${shader.dartClassName}(gpu.ShaderLibrary shaderLibrary)');
    o.writeln('    : super(');
    o.writeln('        bytesPerVertex: ${shader.bytesPerVertex},');
    o.writeln('        shader: ${_generateShaderLibraryGetter(shader)},');
    o.writeln('      );');
    o.writeln('');
    ...
    for (var i = 0; i < shader.attributes.length; i++) {
      final attribute = shader.attributes.elementAt(i);
      o.write('required ${attribute.dartType} ${attribute.dartName()}');
      if (attribute != shader.attributes.last) o.write(', ');
    }
    o.writeln('}) {');
    var offset = 0;
  ...
  for (final shader in fragmentShaders) {
    // -- Class declaration
    o.writeln('/// Generated bindings for the fragment shader `${shader.name}`');
    o.writeln('class ${shader.dartClassName} extends FragmentShaderBindings {');
    o.writeln('  ${shader.dartClassName}(gpu.ShaderLibrary shaderLibrary)');
    o.writeln('    : super(');
    o.writeln('        shader: ${_generateShaderLibraryGetter(shader)},');
    o.writeln('      );');
  ...
  // Generate shader pipelines
  final pipelines = <String, (ParsedShaderVertex, ParsedShaderFragment)>{};

  for (final fragmentShader in fragmentShaders) {
    for (final vertexShader in vertexShaders) {
      if (fragmentShader.name == vertexShader.name) {
        pipelines[fragmentShader.name] = (vertexShader, fragmentShader);
      }
    }
  }

  for (final pipeline in pipelines.entries) {
    final name = pipeline.key;

    final vertexShader = pipeline.value.$1;
    final fragmentShader = pipeline.value.$2;
    ...
    // -- UBO setter --
    o.writeln(_generateShaderUboSetter(vertexShader, fragmentShader));
